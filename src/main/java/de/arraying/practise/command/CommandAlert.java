 ...
 */
public final class CommandAlert extends AbstractCommand {

    /**
     * Creates the command.
     */
    public CommandAlert() {
        super("alert", Rank.MOD, Target.BOTH, "a", "broadcast", "bcast", "bc");
    }
    ...
    public void execute(NexusCommandContext context) {
        if(context.getArgs().length == 0) {
            context.reply("Please provide the message.");
            return;
        }
        String message = String.join(" ", context.getArgs());
        Bukkit.broadcastMessage(ChatColor.YELLOW + "[ALERT] " + message);
        Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "title @a subtitle {\"color\":\"yellow\",\"text\":\"" + message + "\"}");
